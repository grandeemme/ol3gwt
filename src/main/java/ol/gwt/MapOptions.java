...
public class MapOptions extends JavaScriptObject {
	...
	/**
	 * The ratio between physical pixels and device-independent pixels (dips) on the device. If undefined then it gets
	...
	/**
	 * the element to listen to keyboard events on. This determines when the KeyboardPan and KeyboardZoom interactions
	 * trigger. For example, if this option is set to document the keyboard interactions will always trigger. If this
	 * option is not specified, the element the library listens to keyboard events on is the map target (i.e. the
	 * user-provided div for the map). If this is not document the target element needs to be focused for key events to
	 * be emitted, requiring that the target element has a tabindex attribute.
	 */
	...
	/**
	 * Renderer. By default, Canvas, DOM and WebGL renderers are tested for support in that order, and the first
	 * supported used. Specify one of the RendererType constants here to use a specific renderer. Note that at present
	 * only the Canvas renderer supports vector data.
	 */
	...
	/**
	 * The container for the map. If not specified at construction time, ol.Map#setTarget must be called for the map to
	 * be rendered.
	 */
