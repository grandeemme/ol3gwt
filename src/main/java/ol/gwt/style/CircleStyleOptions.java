...
public class CircleStyleOptions extends JavaScriptObject {
	...
	/**
	 * If true integral numbers of pixels are used as the X and Y pixel coordinate when drawing the circle in the output
	 * canvas. If false fractional numbers may be used. Using true allows for "sharp" rendering (no blur), while using
	 * false allows for "accurate" rendering. Note that accuracy is important if the circle's position is animated.
	 * Without it, the circle may jitter noticeably. Default value is true.
	 *
	 * @param snapToPixel
	 */
